...
class EditFamilyInformationScreen extends StatelessWidget {
  final ProfileController controller = Get.find();

  EditFamilyInformationScreen({super.key});
  ...
  void _validateAndUpdate() {
    // Logic from your original State class
    bool hasErrors = false;

    if (controller.mUserNameController.value.text.isEmpty) {
      controller.userNameValidator.value = true;
      hasErrors = true;
    } else {
      controller.userNameValidator.value = false;
    ...
    // Add your other specific validations here...
  ...
  Widget _fullView(bool isWeb) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            CustomAppBar(title: 'Edit Family Information', isWeb: isWeb),
    ...
    return Obx(() {
      return Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildSectionTitle(isWeb, 'Spouse Details'),
            SizedBox(height: isWeb ? 16 : 16.h),
            _buildInputField(
              isWeb: isWeb,
              label: 'Name',
              controller: controller.mUserNameController.value,
              errorText: controller.userNameValidator.isTrue ? "Name required" : null,
              icon: Icons.person_outline,
              hint: "Enter Name",
              keyboardType: TextInputType.name,
            ),
            ...
            _buildInputField(
              ...
              label: 'Phone Number',
              controller: controller.mPhoneNoController.value,
              errorText: controller.phoneNumberValidator.isTrue ? controller.sePhoneNumberValidator.value : null,
              icon: Icons.phone_outlined,
              hint: "Enter Phone Number",
              keyboardType: TextInputType.number,
            ),
            SizedBox(height: isWeb ? 30 : 30.h),
            _buildSectionTitle(isWeb, 'Children Details'),
            SizedBox(height: isWeb ? 16 : 16.h),
            _buildInputField(
              isWeb: isWeb,
              label: 'Child One (Name / Age)',
    ...
    return Column(
      ...
      children: [
        TextInputWidget(
          ...
          showFloatingLabel: true,
          prefixIcon: icon,
          borderColor: errorText != null ? Colors.red.withOpacity(0.5) : null,
        ),
        if (errorText != null) ...[
          SizedBox(height: isWeb ? 6 : 6.h),
          Padding(
            padding: EdgeInsets.only(left: isWeb ? 12 : 12.w),
            child: Row(
              children: [
