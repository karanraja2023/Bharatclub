...
class EventDetailsOneScreen extends GetView<EventDetailsOneController> {
      ...
      child: Obx(
        () => Column(
          ...
          children: [
            ...
            // Event Schedule Card
            _buildScheduleCard(isWeb),

            SizedBox(height: 24),

            // Sponsors Section
            sponsorCarousel(context, isWeb),

            SizedBox(height: 32),

            // Form Sections
            _buildSectionHeader("Primary Contact", Icons.person, isWeb),
            SizedBox(height: 16),
            _buildLabel("Primary Name", isWeb),
            TextInputWidget(
              isWeb: isWeb,
              placeHolder: "",
              controller: controller.mPrimaryNameController.value,
            ...
            // Adult Details
            _buildSectionHeader("Adult Details", Icons.people, isWeb),
            SizedBox(height: 16),
            _buildLabel(
              "Number of Adults (Age ${controller.mMemberAdultAge.value} and above)",
              isWeb,
            ...
            _buildQuantityInput(
              controller.mNumberOfAdultsController.value,
            ...
            if (controller.mMemberChildStatus.value == '1') ...[
              SizedBox(height: 32),
              _buildSectionHeader("Children Details", Icons.child_care, isWeb),
              SizedBox(height: 16),
              _buildLabel(
                "Number of Children (Age ${controller.mMemberChildAge.value} and above)",
                isWeb,
              ...
              SizedBox(height: 16),
              _buildLabel("Number of Children (Age 6 and below)", isWeb),
              _buildQuantityInput(
                controller.mNumberOfChildrenAge6BelowController.value,
                "Enter number of children",
                isWeb,
            ...
            // Guest Details
            _buildSectionHeader("Guest Details", Icons.group_add, isWeb),
            SizedBox(height: 16),
            _buildLabel(
              "Number of Adults (Age ${controller.mGuestAdultAge.value} and above)",
              isWeb,
            ...
            _buildQuantityInput(
              controller.mNumberOfGuestAdultsController.value,
              "Enter number of guest adults",
              isWeb,
            ),
            ...
            if (controller.mGuestChildStatus == '1') ...[
              ...
              _buildQuantityInput(
                controller.mNumberOfGuestChildrenAge12AboveController.value,
                "Enter number of guest children",
                isWeb,
              ),
              SizedBox(height: 16),
              _buildLabel("Number of Children (Age 6 and below)", isWeb),
            ...
            // Proceed Button
            _buildProceedButton(context, isWeb),
            SizedBox(height: 20),
          ],
        ),
      ),
  ...
  Widget _buildScheduleCard(bool isWeb) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.cAppColorsBlue.withOpacity(0.2)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Row(
            children: [
              _dateInfoColumn("From", controller.mStartDate.value),
              Container(
                height: 40,
                width: 1,
                color: AppColors.cAppColorsBlue.withOpacity(0.3),
              ...
              _dateInfoColumn("To", controller.mEndDate.value),
            ],
          ),
        ],
      ),
  ...
  Widget _dateInfoColumn(String label, String date) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
            label,
            style: getTextMedium(colors: Colors.grey[600]!, size: 13),
          ),
  ...
  Widget _buildLabel(String text, bool isWeb) {
    return Padding(
  ...
  Widget _buildSectionHeader(String title, IconData icon, bool isWeb) {
    return Row(
      children: [
        Container(
          ...
          child: Icon(icon, size: 20, color: AppColors.cAppColorsBlue),
        ),
  ...
  Widget _buildProceedButton(BuildContext context, bool isWeb) {
    return InkWell(
      onTap: () => controller.navigateToNextScreen(mEventModule, context),
      child: Container(
        height: 56,
        decoration: BoxDecoration(
          gradient: LinearGradient(
        ...
        child: Row(
          ...
          children: [
            Text(
              "Proceed",
              style: getTextSemiBold(colors: Colors.white, size: 16),
            ),
            const SizedBox(width: 8),
  ...
  Widget sponsorCarousel(BuildContext context, bool isWeb) {
    final PageController pageController = PageController();
    controller.startTimer(pageController);

    return Obx(() {
      if (controller.sponsorList.isEmpty) return const SizedBox.shrink();

      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSectionHeader("Our Sponsors", Icons.business, isWeb),
          const SizedBox(height: 16),
          SizedBox(
            height: isWeb ? 250 : MediaQuery.of(context).size.width * 0.6,
            child: PageView.builder(
              controller: pageController,
              itemCount: controller.sponsorList.length,
      ...
      child: Container(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            fit: StackFit.expand,
            children: [
              Image.network(sponsor.fileUrl, fit: BoxFit.cover),
              Positioned(
                bottom: 0,
                left: 0,
                right: 0,
                child: Container(
                  ...
                  child: Text(
                    sponsor.companyName,
                    style: getTextBold(colors: Colors.white, size: 16),
                    textAlign: TextAlign.center,
                  ),
                ),
              ),
