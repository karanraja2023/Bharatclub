...
class _TermsAndConditionsPageState extends State<TermsAndConditionsPage> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        // Breakpoint matching ProfileScreen logic
          ...
          return Scaffold(
            ...
            body: Center(
              child: Container(
                ...
                child: ClipRect(
                  child: _mainContent(isWeb),
                ),
              ),
            ),
          );
        } else {
          return _mainContent(false);
        }
      },
    );
  }

  Widget _mainContent(bool isWeb) {
    return FocusDetector(
      onVisibilityLost: () {
        Get.delete<TermsAndConditionsController>();
      },
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  padding: EdgeInsets.symmetric(
                    horizontal: isWeb ? 16 : 10.w,
                    vertical: isWeb ? 10 : 10.h,
                  ),
                  child: _termsAndConditionsContent(isWeb),
                ),
              ),
              _actionButtons(isWeb),
              SizedBox(height: isWeb ? 20 : 10.h),
            ],
          ),
        ),
      ),
  ...
  Widget _termsAndConditionsContent(bool isWeb) {
    return Card(
      elevation: 2,
      color: Colors.white,
      shape: RoundedRectangleBorder(
      ...
      child: Padding(
        padding: EdgeInsets.all(isWeb ? 20 : 15.w),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
  ...
  Widget _actionButtons(bool isWeb) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: isWeb ? 20 : 15.w),
      child: Row(
        children: [
          // Disagree Button
          Expanded(
            child: _buildButton(
              label: "Disagree",
              isActive: _isAtBottom,
              activeColor: Colors.redAccent,
              onPressed: () => Get.offNamed(AppRoutes.loginScreen),
              isWeb: isWeb,
            ),
          ),
          SizedBox(width: isWeb ? 15 : 15.w),
          // Agree Button
          Expanded(
            child: _buildButton(
              label: "Agree",
              isActive: _isAtBottom,
              activeColor: AppColors.cAppColors,
              onPressed: () => Get.find<TermsAndConditionsController>().webView(),
              isWeb: isWeb,
            ),
          ),
        ],
  ...
  Widget _buildButton({
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        ...
        minimumSize: Size(double.infinity, isWeb ? 45 : 45.h),
      ),
      child: Text(
        label,
        style: getTextSemiBold(
...
1. Introduction
This document outlines the data collection, usage, and protection policies for the Bharat Club mobile application...

[... rest of your terms text ...]
''';
}