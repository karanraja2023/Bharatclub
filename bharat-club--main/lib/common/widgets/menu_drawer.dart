...
import 'package:mobileapp/common/controller/side_menu_contoller.dart';
...
import 'package:mobileapp/screens/qr_code_scan/view/qr_access_service.dart';
import 'package:mobileapp/utils/app_text.dart';
import 'package:mobileapp/utils/device_info_helper.dart';
...
class _CustomMenuDrawerState extends State<CustomMenuDrawer>
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      // On Web, the drawer should have a fixed width (e.g., 300px)
      width: isWeb ? 300 : 320.w,
      child: Drawer(
        backgroundColor: AppColors.secondaryGreen,
        child: Padding(
          padding: EdgeInsets.only(top: isWeb ? 20 : 40.h),
          child: Column(
            children: [
              // 1. Logo Section
              ...
              Expanded(
                child: ListView.builder(
                  padding: EdgeInsets.symmetric(vertical: isWeb ? 10 : 8.h),
                  itemCount: menuSections.length,
                  itemBuilder: (context, sectionIndex) {
                    final section = menuSections[sectionIndex];
                    return Column(
                      children: section.items
                          .map((item) => _buildMenuItem(item, isWeb))
                          .toList(),
                    );
                  },
                ),
        ...
        builder: (context, child) {
          return Transform.scale(
            ...
            child: ClipRRect(
              ...
              child: Image.asset(
                currentLogo,
                width: isWeb ? 180 : 200.w,
                height: isWeb ? 60 : 70.h,
                fit: BoxFit.contain, // Better for web logos
              ),
            ),
          );
        },
  ...
  Widget _buildMenuItem(MenuItem item, bool isWeb) {
    final isActive = _isActiveRoute(item.route);
    ...
    return Obx(() {
      ...
      return Container(
        margin: EdgeInsets.symmetric(
          horizontal: isWeb ? 12 : 8.w,
          vertical: isWeb ? 4 : 1.h,
        ),
        child: Material(
          color: AppColors.transparent,
          child: InkWell(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Icon(
                    item.icon,
                    color: isActive ? AppColors.white : Colors.grey.shade400,
                    size: isWeb ? 20 : 20.sp,
                  ),
  ...
  Widget _buildLogoutSection(bool isWeb) {
    return Container(
      padding: EdgeInsets.all(isWeb ? 20 : 16.w),
      child: Container(
        width: double.infinity,
        height: isWeb ? 50 : 56.h,
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [AppColors.primaryGreen, AppColors.secondaryGreen],
          ),
          borderRadius: BorderRadius.circular(isWeb ? 12 : 16.r),
        ),
        child: InkWell(
          onTap: () => mHomeController.logout(),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(
